...
const net = require('net') // 新增：用于检测端口是否被占用

// 新增：检测端口是否可用（判断开发者工具是否已启动并监听该端口）
function checkPortIsUsed(port) {
  return new Promise(resolve => {
    const client = new net.Socket()
    client.setTimeout(3000) // 超时时间3秒
    client.on('connect', () => {
      client.destroy()
      resolve(true) // 端口被占用（开发者工具已运行）
    })
    client.on('timeout', () => {
      client.destroy()
      resolve(false) // 超时，端口未占用
    })
    client.on('error', () => {
      client.destroy()
      resolve(false) // 连接失败，端口未占用
    })
...
async function main() {
  ...
  // 新增：先检测端口是否已被占用（开发者工具是否已运行）
  const isPortUsed = await checkPortIsUsed(autoPort)
  if (isPortUsed) {
    console.log(`✅ 检测到 ${autoPort} 端口已被占用，开发者工具已运行，跳过重启\n`)
  } else {
    console.log(`🔄 ${autoPort} 端口未占用，启动开发者工具自动化模式...\n`)
    // 仅当端口未占用时，才执行auto命令启动开发者工具
